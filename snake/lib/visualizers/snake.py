...
class Visualizer:
    ...
    def visualize_training_box(self, output, batch):
        ...
        label = output['detection'][:, 5].detach().cpu().numpy().astype(int)
        ...
        for i in range(len(ex)):
            # if label[i] == 4:
                ...
                poly = ex[i]
                poly = np.append(poly, [poly[0]], axis=0)
                ax.plot(poly[:, 0], poly[:, 1], color=color, linewidth=0.8)  
        ...
        plt.savefig("/data/workspace/zhuhanqi/snake/result/%d.png" % Randnum)
        plt.close()
