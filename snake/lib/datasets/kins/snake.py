...
class Dataset(data.Dataset):
    def __init__(self, ann_file, data_root, split):
        ...
        ############################ for training 
        # mask = np.zeros(  self.anns.shape[0] , dtype=bool)
        # split = np.loadtxt('/root/exchange/snake/data/split.txt')
        # mask[:] = False
        # for i in range( self.anns.shape[0] ):
        #     img_id = self.anns[i]
        ...
        #     #print(int(tempfilename)   ) 
        #     if ((int(tempfilename)) in split ) == True:
        #         mask[i] = True
        # self.anns = self.anns[mask]
        ###################################
 
        self.anns = self.anns[:500] if self.split == 'mini' else self.anns 
    ...
    def process_info(self, img_id):
        ...
        path = os.path.join(self.data_root, self.coco.loadImgs(int(img_id) * 20 )[0]['file_name'])   
        print('hahahha _fuck u' , path )
        return anno, path, img_id

    def read_original_data(self, anno, path):
 
        path = '/data/workspace/zhuhanqi/snake/' + path
        img = cv2.imread(path)
    ...
    def __getitem__(self, index):
        ...
        # detection
        output_h, output_w = inp_out_hw[2:]
        ct_hm = np.zeros([cfg.heads.ct_hm, output_h, output_w], dtype=np.float32)  ## category number 
        wh = []
        ct_cls = []
        ct_ind = []
        ...
        ct_num = len(ct_ind)
        ## VPFNet
        meta = {'center': center, 'scale': scale, 'img_id': img_id,'path': path , 'tempfilename': tempfilename, 'ann': ann, 'ct_num': ct_num}
