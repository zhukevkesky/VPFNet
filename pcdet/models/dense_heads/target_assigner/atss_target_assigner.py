    ...
    def assign_targets_single(self, anchors, gt_boxes, gt_classes):
        ...
        if self.match_height:
            ious = iou3d_nms_utils.boxes_iou3d_gpu(
                anchors[:, 0:7], gt_boxes[:, 0:7])  # (N, M)
        else:
            ious = iou3d_nms_utils.boxes_iou_bev(
                anchors[:, 0:7], gt_boxes[:, 0:7])

        distance = (anchors[:, None, 0:3] -
                    gt_boxes[None, :, 0:3]).norm(dim=-1)  # (N, M)
        ...
        gt_boxes_of_each_anchor = gt_boxes[:, :].repeat(
            self.topk, 1)  # (KxM, 7)
        ...
        is_in_gt = ((xy_local <= lw / 2) & (xy_local >= -lw / 2)
                    ).all(dim=-1).view(-1, num_gt)  # (K, M)
        ...
        INF = -0x7FFFFFFF
        ious_inf = torch.full_like(
            ious, INF).t().contiguous().view(-1)  # (MxN)
