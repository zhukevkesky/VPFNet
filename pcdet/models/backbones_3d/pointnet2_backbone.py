...
class PointNet2MSG(nn.Module):
    def __init__(self, model_cfg, input_channels, **kwargs):
        ...
        for k in range(self.model_cfg.FP_MLPS.__len__()):
            pre_channel = self.model_cfg.FP_MLPS[k + 1][-1] if k + \
                1 < len(self.model_cfg.FP_MLPS) else channel_out
    ...
    def forward(self, batch_dict):
        ...
        features = features.view(
            batch_size, -1, features.shape[-1]).permute(0, 2, 1) if features is not None else None
        ...
        point_features = l_features[0].permute(
            0, 2, 1).contiguous()  # (B, N, C)
        batch_dict['point_features'] = point_features.view(
        ...
        for i in range(len(self.SA_modules)):
            ...
            for k in range(batch_size):
                if len(l_xyz) == 1:
                    ...
                    cur_xyz = l_xyz[-1][k *
                                        last_num_points: (k + 1) * last_num_points]
                ...
                if cur_xyz.shape[0] < self.num_points_each_layer[i]:
                    empty_num = self.num_points_each_layer[i] - \
                        cur_xyz.shape[1]
                    cur_pt_idxs[0, -empty_num:] = cur_pt_idxs[0, :empty_num]
                new_xyz_list.append(cur_xyz[cur_pt_idxs])
            new_xyz = torch.cat(new_xyz_list, dim=0)

            new_xyz_batch_cnt = xyz.new_zeros(
                batch_size).int().fill_(self.num_points_each_layer[i])
