...
class DatasetTemplate(torch_data.Dataset):
    ...
    def project_depth_to_points(self,  calib, depth, conf, max_high):

        rows, cols = depth.shape
        c, r = np.meshgrid(np.arange(cols), np.arange(rows))

        rect = calib.img_to_rect_color(c, r, depth, conf)
        cloud = calib.rect_to_lidar_color(rect)

        valid = (cloud[:, 0] > 1) & (
            cloud[:, 2] < max_high) & (cloud[:, 0] < 77)
        cloud = cloud[valid]
    ...
    def prepare_data(self, data_dict):
        ...
        if data_dict.get('gt_boxes', None) is not None:
            ...
            data_dict['gt_difficulty'] = data_dict['gt_difficulty'][selected]
        ...
        for key, val in data_dict.items():
            try:
                if key in ['voxels', 'voxel_num_points']:
                    ...
                    ret[key] = val
                elif key in ['calib', 'torch_calib']:

                    ret[key] = val
                elif key in ['gt_difficulty']:

                    max_gt = max([len(x) for x in val])
                    batch_gt_boxes3d = np.zeros(
                        (batch_size, max_gt), dtype=np.float32)
                    batch_gt_boxes3d -= 100
                    for k in range(batch_size):
                        batch_gt_boxes3d[k, :val[k].__len__()] = val[k]
                    ret[key] = batch_gt_boxes3d

                elif key in ['box']:  # B , N , M
                    max_gt = max([len(x) for x in val])
                    temp2 = np.zeros(
                        (batch_size, max_gt,  6, 2), dtype=np.float32)
                    for k in range(batch_size):
                        temp2[k, :val[k].__len__(), :, :] = val[k]
                    ret[key] = temp2
