...
class VoxelQuery2(Function):
    ...
    def forward(ctx, max_range: int, radius: float, nsample: int, xyz: torch.Tensor, \
        ...
        pointnet2.voxel_query_wrapper2(M, Z, Y, X, nsample, radius, z_range, y_range, x_range, \
...
voxel_query2 = VoxelQuery2.apply

class VoxelQueryAndGrouping2(nn.Module):
    ...
    def forward(self, new_coords: torch.Tensor, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
        ...
        idx1, empty_ball_mask1 = voxel_query2(self.max_range, self.radius, self.nsample, xyz, new_xyz, new_coords, voxel2point_indices)  # xyz, new_coords, point_indices, idx
