...
__global__ void voxel_query_kernel_stack2(int M, int R1, int R2, int R3, int nsample, 
    ...
    int new_coords_indicator = new_coords[4];

    int cnt = 0;
    // int cnt2 = 0;
    
    // if (new_coords_indicator < 0.3){
...
void voxel_query_kernel_launcher_stack2(int M, int R1, int R2, int R3, int nsample,
    float radius, int z_range, int y_range, int x_range, const float *new_xyz, 
    const float *xyz, const int *new_coords, const int *point_indices, int *idx) {
    // :param new_coords: (M1 + M2 ..., 4) centers of the voxel query
    // :param point_indices: (B, Z, Y, X) 
    ...
    voxel_query_kernel_stack2<<<blocks, threads>>>(M, R1, R2, R3, nsample, radius, z_range, y_range, x_range, new_xyz, xyz, new_coords, point_indices, idx);
    // cudaDeviceSynchronize();  // for using printf in kernel function 
