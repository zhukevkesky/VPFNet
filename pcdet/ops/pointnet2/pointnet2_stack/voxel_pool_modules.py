...
class NeighborStereoSAModuleMSG(nn.Module):
    ...
    def __init__(self, *, query_ranges: List[List[int]], radii: List[float],
        ...
        for i in range(len(query_ranges)):
            ...
            nsample = nsamples[i]
            radius = radii[i]
            self.groupers.append(voxel_query_utils.VoxelQueryAndGrouping2(
                max_range, radius, nsample))
            mlp_spec = mlps[i]
    ...
    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt,
        ...
        new_coords = new_coords[:, [0, 3, 2, 1, 4]].contiguous()
        new_features_list = []
        for k in range(len(self.groupers)):
            ...
            # new_features = self.mlps_pos[k](new_features)
...
class NeighborVoxelSAModuleMSG(nn.Module):
    ...
    def __init__(self, *, query_ranges: List[List[int]], radii: List[float],
        ...
        for i in range(len(query_ranges)):
            ...
            nsample = nsamples[i]
            radius = radii[i]
            self.groupers.append(voxel_query_utils.VoxelQueryAndGrouping(
                max_range, radius, nsample))
            mlp_spec = mlps[i]
