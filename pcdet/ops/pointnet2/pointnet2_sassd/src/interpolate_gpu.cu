...
void three_nn_kernel_launcher_fast(int n, int m, const float *unknown,
    ...
    three_nn_kernel_fast<<<blocks, threads, 0>>>(n, m, unknown, known, dist2, idx);
...
void three_interpolate_kernel_launcher_fast(int c, int m, int n,
   ...
   // points: (M, C)
    ...
    dim3 threads(THREADS_PER_BLOCK);
    three_interpolate_kernel_fast<<<blocks, threads, 0>>>(c, m, n, points, idx, weight, out);
...
void three_interpolate_grad_kernel_launcher_fast(int c, int n, int m, const float *grad_out,
    ...
    three_interpolate_grad_kernel_fast<<<blocks, threads,0>>>(c, n, m, grad_out, idx, weight, grad_points);
